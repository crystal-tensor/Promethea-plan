#!/usr/bin/env python3
"""Independently audit R181 without importing Qiskit or its comparator."""
...
METHOD = "b4_b8_r181_independent_active_limb_oracle_v0"
...
SOURCE_PATH = "results/B4_B8_R181_active_limb_replay_v0.json"
WORKER_DIR = "results/B4_B8_R181_active_limb_replay"
R160_ANALYSIS_PATH = (
...
RESULT_PATH = "results/B4_B8_R181_independent_active_limb_oracle_v0.json"
REPORT_PATH = "research/B4_B8_R181_independent_active_limb_oracle.md"
...
def validate_hash_field(payload: dict[str, Any], field: str, label: str) -> str:
    ...
    if not observed or observed != canonical_hash(body):
        raise ValueError(f"R181 independent {label} hash mismatch")
    return str(observed)
...
def exact_leaf_oracle(
    ...
    if not scored:
        raise ValueError("R181 independent oracle found no feasible mapping")
...
def main() -> int:
    ...
    if (root / RESULT_PATH).exists() or (root / REPORT_PATH).exists():
        raise ValueError("R181 independent oracle evidence already exists")
    ...
    for section in ("source_bindings", "tool_bindings"):
        for binding in contract[section].values():
            ...
            if not path.exists() or file_sha256(path) != binding["sha256"]:
                raise ValueError(
                    f"R181 independent binding mismatch: {binding['path']}"
                )
    manifests = []
    ...
    for manifest in [row for row in manifests if row["kind"] == "small-gap"]:
        for case in manifest["case_summaries"]:
            ...
            if cell in oracle_by_cell and oracle_by_cell[cell] != oracle:
                raise ValueError(f"R181 independent descriptor disagreement: {cell}")
    ...
    requirements = [
        ...
        (
            ...
            and "b4_b8_r181_active_limb_replay" not in sys.modules,
        ),
        ("P12", preregistration_matches),
    ]
    passed = all(value for _, value in requirements)
    result = {
        "title": "B4/B8/B10 R181 independent active-limb superaccumulator oracle",
        "version": 0,
        "method": METHOD,
        ...
        "source_target_id": "T-B4-002dp/T-B8-003dt/T-B10-009df-r181-oracle",
        ...
        "summary": {
            ...
            "r181_executor_imported": "b4_b8_r181_active_limb_replay" in sys.modules,
        ...
        "claim_boundary": {
            "what_is_supported": "a Qiskit-free standard-library reproduction of every frozen R181 mapping expectation, sub-ULP exact oracle, hash, timing ratio, and peak-RSS ratio",
