#!/usr/bin/env python3
"""Create a hash manifest for the downloadable R182 evidence bundle."""
...
ORACLE_PATH = "results/B4_B8_R182_independent_cost_oracle_v0.json"
WORKER_DIR = "results/B4_B8_R182_score_cost_attribution_replay"
BUILD_MANIFEST_PATH = "research/source_lineage/Qiskit_2_4_1_R182_score_cost_linux_x86_64_build_manifest.json"
BINARY_PATH = (
...
BUNDLE_PATH = "results/B4_B8_R182_score_cost_attribution_bundle_v0.json"
...
def payload_hash(payload: dict[str, Any]) -> str:
    ...
    if not observed or observed != canonical_hash(body):
        raise ValueError("R182 bundle source payload mismatch")
    return str(observed)
...
def main() -> int:
    ...
    if (root / BUNDLE_PATH).exists():
        raise ValueError("R182 bundle manifest already exists")
    ...
    required = [
        ...
        Path(RESULT_PATH),
        Path(ORACLE_PATH),
        Path("research/B4_B8_R182_score_cost_attribution.md"),
        Path("research/B4_B8_R182_independent_cost_oracle.md"),
    ]
    required.extend(
    ...
    for relative in required:
        path = root / relative
        if not path.is_file():
            raise ValueError(f"R182 bundle artifact missing: {relative}")
        artifacts.append(
            {
                "path": str(relative),
    ...
    manifest = {
        "title": "B4/B8/B10 R182 score-cost Linux x86-64 evidence bundle",
        "version": 0,
        "method": "b4_b8_r182_score_cost_attribution_bundle_v0",
    ...
    if (
        ...
        raise ValueError("R182 bundle worker or GitHub-run boundary mismatch")
