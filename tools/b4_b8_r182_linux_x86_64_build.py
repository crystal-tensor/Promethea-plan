#!/usr/bin/env python3
"""Build the preregistered R182 Qiskit accelerator on Linux x86-64."""
...
METHOD = "b4_b8_r182_score_cost_attribution_linux_x86_64_build_v0"
...
PATCH_PATH = "research/source_lineage/Qiskit_2_4_1_R182_score_cost_attribution.patch"
BINARY_PATH = (
    "research/source_lineage/Qiskit_2_4_1_R182_score_cost_pyext.x86_64-linux-gnu.so"
)
MANIFEST_PATH = "research/source_lineage/Qiskit_2_4_1_R182_score_cost_linux_x86_64_build_manifest.json"
LOG_DIR = "research/source_lineage/R182_score_cost_linux_x86_64_build_logs"
EXPECTED_SOURCE_HASHES = {
    ...
    "crates/transpiler/src/passes/vf2/vf2_layout.rs": (
        "acf3d2015d40f1b13bde96b6da4256f733ffc2c796662dbe63177698de0a8ed5"
    ),
}
...
def validate_preregistration(
    ...
    if int(time.time()) <= created:
        raise ValueError("R182 build must start after public preregistration")
    public = contract["public_preregistration"]
    if args.preregistration_discussion != public["discussion"]:
        raise ValueError("R182 preregistration discussion boundary mismatch")
    if args.preregistration_created_at != public["created_at"]:
        raise ValueError("R182 preregistration creation time mismatch")
    if contract.get("execution_started") is not False:
        raise ValueError("R182 contract must remain an unopened record")
    current_commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=root, text=True
    ).strip()
    if args.preregistration_commit != current_commit:
        raise ValueError("R182 runner commit does not match the preregistration commit")
    if (
        os.environ.get("GITHUB_ACTIONS") == "true"
        and os.environ.get("GITHUB_SHA") != current_commit
    ):
        raise ValueError("R182 GitHub Actions checkout does not match HEAD")
    ...
    if ancestor.returncode != 0:
        raise ValueError("R182 build commit predates the public count amendment")
...
def run_step(
    ...
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"R182 build step failed: {step_id}\n{completed.stdout}\n{completed.stderr}"
        )
    return record
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("R182 contract identity mismatch")
    ...
    for relative in contract.get("result_paths_must_be_absent", []):
        if (root / relative).exists():
            raise ValueError(
                f"R182 evidence existed before Linux execution: {relative}"
            )
    for relative in contract.get("build_output_paths_created_before_replay", []):
        if (root / relative).exists():
            raise ValueError(f"R182 build output existed before build: {relative}")
    if platform.system() != "Linux" or platform.machine() not in {"x86_64", "amd64"}:
        raise ValueError(
            f"R182 requires Linux x86-64, observed {platform.system()} {platform.machine()}"
        )

    source = Path("/tmp/prometheus-r182-qiskit-source")
    if source.exists():
    ...
    if observed_source_hashes != EXPECTED_SOURCE_HASHES:
        raise ValueError("R182 patched source hashes do not match the frozen patch")
    ...
    if library_name != "qiskit_pyext" or "cdylib" not in crate_types:
        raise ValueError(
            "R182 Qiskit pyext metadata does not declare qiskit_pyext cdylib"
        )
    ...
    if not candidate.is_file():
        raise ValueError(
            f"R182 source-declared release artifact is missing: {candidate}"
        )
    ...
    if "x86-64" not in file_description or "shared object" not in file_description:
        raise ValueError(f"R182 built unexpected binary: {file_description}")

    qiskit_spec = importlib.util.find_spec("qiskit")
    if qiskit_spec is None or not qiskit_spec.submodule_search_locations:
        raise ValueError("R182 cannot locate the pinned installed Qiskit package")
    installed_qiskit = Path(
        next(iter(qiskit_spec.submodule_search_locations))
    ).resolve()
    smoke_overlay = Path("/tmp/prometheus-r182-score-cost-import-smoke")
    ...
    smoke_code = "\n".join(
        [
            ...
            "names = ['vf2_layout_pass_average_exact_score_cost_traced', 'vf2_layout_pass_average_fixed_exact_score_cost_traced', 'vf2_layout_pass_average_active_fixed_exact_score_cost_traced']",
            "entries = {name: callable(getattr(accelerator.vf2_layout, name, None)) for name in names}",
            "print(json.dumps({'accelerator_path': str(path), 'accelerator_sha256': hashlib.sha256(path.read_bytes()).hexdigest(), 'configuration_type': VF2PassConfiguration.__name__, 'cost_trace_entry_points': entries}, sort_keys=True))",
        ]
    )
    steps.append(
        run_step(
    ...
    if (
        ...
        or not all(import_smoke.get("cost_trace_entry_points", {}).values())
    ):
        raise ValueError("R182 Python import smoke did not bind the built accelerator")
    ...
    manifest = {
        "title": "Qiskit 2.4.1 R182 score-cost Linux x86-64 build manifest",
        "version": 0,
        "method": METHOD,
