#!/usr/bin/env python3
"""Build the preregistered R179 Qiskit accelerator on Linux x86-64."""
...
METHOD = "b4_b8_r179_linux_x86_64_build_v0"
SOURCE_COMMIT = "0fd015a22b84c9082173597a5d2304dc0aaec08c"
SOURCE_URL = "https://github.com/Qiskit/qiskit.git"
PROTOCOL_PATH = "results/B4_B8_R179_linux_x86_64_protocol_v0.json"
CONTRACT_PATH = "benchmarks/B4_B8_R179_linux_x86_64_contract_v0.json"
PATCH_PATH = "research/source_lineage/Qiskit_2_4_1_R176_fixed_superaccumulator.patch"
BINARY_PATH = (
    "research/source_lineage/"
    "Qiskit_2_4_1_R179_fixed_superaccumulator_pyext.x86_64-linux-gnu.so"
)
MANIFEST_PATH = (
    "research/source_lineage/Qiskit_2_4_1_R179_linux_x86_64_build_manifest.json"
)
LOG_DIR = "research/source_lineage/R179_linux_x86_64_build_logs"
...
def validate_payload(payload: dict[str, Any], label: str) -> str:
    ...
    if not observed or observed != canonical_hash(body):
        raise ValueError(f"R179 {label} payload hash mismatch")
    return str(observed)
...
def validate_preregistration(
    ...
    if int(time.time()) <= created:
        raise ValueError("R179 build must start after public preregistration")
    if not args.preregistration_discussion.startswith(
        "https://github.com/crystal-tensor/Prometheus-plan/discussions/"
    ):
        raise ValueError("R179 preregistration discussion boundary mismatch")
    if contract.get("execution_started") is not False:
        raise ValueError("R179 contract must remain an unopened record")
    current_commit = command_output(["git", "rev-parse", "HEAD"])
    if args.preregistration_commit != current_commit:
        raise ValueError("R179 runner commit does not match the preregistration commit")
    if (
        os.environ.get("GITHUB_ACTIONS") == "true"
        and os.environ.get("GITHUB_SHA") != current_commit
    ):
        raise ValueError("R179 GitHub Actions checkout does not match HEAD")
...
def run_step(
    ...
    *,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"R179 build step failed: {step_id}\n{completed.stdout}\n{completed.stderr}"
        )
    return record
...
def main() -> int:
    ...
    if protocol.get("method") != "b4_b8_r179_linux_x86_64_protocol_v0":
        raise ValueError("R179 protocol identity mismatch")
    if contract.get("contract_id") != "B4-B8-R179-linux-x86-64-contract-v0":
        raise ValueError("R179 contract identity mismatch")
    if contract.get("protocol_payload_hash") != protocol.get("payload_hash"):
        raise ValueError("R179 protocol binding mismatch")
    for section in ("source_bindings", "tool_bindings"):
        for binding in contract.get(section, {}).values():
            ...
            if not path.exists() or file_sha256(path) != binding["sha256"]:
                raise ValueError(f"R179 binding mismatch: {binding['path']}")
    for relative in contract.get("result_paths_must_be_absent", []):
        if (root / relative).exists():
            raise ValueError(
                f"R179 evidence existed before Linux execution: {relative}"
            )
    if platform.system() != "Linux" or platform.machine() not in {"x86_64", "amd64"}:
        raise ValueError(
            f"R179 requires Linux x86-64, observed {platform.system()} {platform.machine()}"
        )

    source = Path("/tmp/prometheus-r179-qiskit-source")
    if source.exists():
    ...
    if observed_source_hashes != EXPECTED_SOURCE_HASHES:
        raise ValueError("R179 patched source hashes do not match R176")
    ...
    if library_name != "qiskit_pyext" or "cdylib" not in crate_types:
        raise ValueError(
            "R179 Qiskit pyext metadata does not declare qiskit_pyext cdylib"
        )
    ...
    if not candidate.is_file():
        raise ValueError(
            f"R179 source-declared release artifact is missing: {candidate}"
        )
    ...
    if "x86-64" not in file_description or "shared object" not in file_description:
        raise ValueError(f"R179 built unexpected binary: {file_description}")

    qiskit_spec = importlib.util.find_spec("qiskit")
    if qiskit_spec is None or not qiskit_spec.submodule_search_locations:
        raise ValueError("R179 cannot locate the pinned installed Qiskit package")
    installed_qiskit = Path(
        next(iter(qiskit_spec.submodule_search_locations))
    ).resolve()
    smoke_overlay = Path("/tmp/prometheus-r179-import-smoke")
    ...
    if (
        ...
        raise ValueError("R179 Python import smoke did not bind the built accelerator")
    ...
    manifest = {
        "title": "Qiskit 2.4.1 R179 Linux x86-64 build manifest",
        "version": 0,
        "method": METHOD,
