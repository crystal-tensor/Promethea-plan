#!/usr/bin/env python3
"""Create a hash manifest for the downloadable R181 evidence bundle."""
...
ORACLE_PATH = "results/B4_B8_R181_independent_active_limb_oracle_v0.json"
...
BUNDLE_PATH = "results/B4_B8_R181_active_limb_bundle_manifest_v0.json"
...
def payload_hash(payload: dict[str, Any]) -> str:
    ...
    if not observed or observed != canonical_hash(body):
        raise ValueError("R181 bundle source payload mismatch")
    return str(observed)
...
def main() -> int:
    ...
    if (root / BUNDLE_PATH).exists():
        raise ValueError("R181 bundle manifest already exists")
    ...
    required = [
        ...
        Path(RESULT_PATH),
        Path(ORACLE_PATH),
        Path("research/B4_B8_R181_active_limb_replay.md"),
        Path("research/B4_B8_R181_independent_active_limb_oracle.md"),
    ]
    required.extend(
    ...
    for relative in required:
        path = root / relative
        if not path.is_file():
            raise ValueError(f"R181 bundle artifact missing: {relative}")
        artifacts.append(
            {
                "path": str(relative),
    ...
    manifest = {
        "title": "B4/B8/B10 R181 active-limb Linux x86-64 evidence bundle",
        "version": 0,
        "method": "b4_b8_r181_active_limb_bundle_v0",
    ...
    if (
        manifest["worker_artifact_count"] != 52
        or not manifest["github_actions_run_url"]
    ):
        raise ValueError("R181 bundle worker or GitHub-run boundary mismatch")
