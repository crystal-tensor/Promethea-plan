...
Current state: **stage 1h evidence hardening; B10-T1 asymptotic access-contract note is complete, the current B3/B5 sampling-access bridge is refuted, and the next theory gate is a B5 same-access sampling-or-DMRG denominator bridge**.
...
| B10 | Boundary of BQP | T-B10-011 asymptotic access-contract note: 2 B3/B5 family contracts, 8 explicit/oracle/sampling/quantum access rows, 5 bridge conditions, current sampling-access bridge refuted, and no general dequantization theorem, sampling-access theorem, BQP separation, or quantum advantage claim. B10-T2 still has the trained-spoofer/proof-gate/transcript/device-noise/Qiskit-Aer/backend-calibrated-style bridge stack. | T-B10-012 must pick the B5 Hubbard response family and either build a comparable sampling/query oracle with variance/preparation costs, or prove the same-access denominator still favors production DMRG/MPS; separately, replace the GenericBackendV2 bridge with real backend properties or hardware verifier execution. | Demote any positive B10-T1 route unless it supplies equivalent access contracts, a comparable sampling/query oracle, and same-access denominator comparisons. |
...
| 3 | Keep B3 demoted unless a rescue mechanism appears; advance B5 from MPS/ALS prototype to mature variational DMRG/MPS evidence or quantum response-kernel cost accounting; advance B6 from formula-derived proxy boundary to crystallographic/DFT/B5-computed descriptors plus expanded post-2008 negatives. |
