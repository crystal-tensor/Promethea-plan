...
Current state: **stage 1x evidence hardening; B1/B7 now has a `gcm_h6` target selector with 3 cone classes and 2 canonical angle classes meeting the 30-occurrence target, B7 still lacks a certified occurrence-removing rewrite, B2 remains a hardware-like leakage negative boundary, and B10 remains a denominator-boundary track**.
...
|---|---|---|---|---|
| B1 | Hardware-aware quantum circuit compression | Virtual-SWAP plus U3 phase-factored T-resource diagnostics; new `gcm_h6` selector counts 270 arbitrary decimal rotations and exposes 3 local CNOT-cone classes, 2 canonical angle classes, and 4 qubit classes that meet B7's 30-occurrence target if a future rewrite removes one per occurrence. | Turn one ranked `gcm_h6` family into a replayable semantic rewrite certificate, remove at least 30 arbitrary rotation occurrences / 600 proxy-T units, then re-run the B7 FT ledger. | Demote if reductions disappear on broader benchmarks or certificate generation cannot scale beyond local rewrite evidence. |
...
| B7 | Fault-tolerance co-design | B1/B2 bridge plus FT synthesis ledger; `w8_21` claim-boundary closure with 43480 optimizer runs, 0 exact replacements, and 0 ledger removal; template-priority gate over 12 retained templates; B1-side `gcm_h6` target selector now identifies concrete cone/angle/qubit families that could meet the 30-occurrence target only if a future semantic certificate removes them. | Improve the B7 minimum row through certified B1 T-resource work, or produce a symbolic KAK/Clifford-scaffold proof / alternate occurrence-removing rewrite for `gcm_h6` that removes enough arbitrary rotations per occurrence. | Demote if B1/B2-linked reductions collapse once physical layout, factory throughput, feed-forward, and occurrence-level synthesis certificates are explicit. |
