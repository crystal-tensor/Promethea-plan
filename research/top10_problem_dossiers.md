...
T-B5-006c/T-B10-014a adds a production implementation triage gate over that
failed contract. It satisfies 6/6 triage readiness conditions, keeps failed
source gates P2-P9 explicit, and splits the next work into 6 PR-sized packets:
W1 production DMRG/MPS denominator, W2 seeded-pressure replacement audit, W3
same-access response-oracle cost ledger, W4 row-contract harness, W5 B10-T1
theorem-boundary integration, and W6 claim-safety/audit wiring. Only W4/W6 are
...
T-B5-006c/T-B10-014a now converts that production-contract failure into a B10-T1
implementation queue: 6 work packets, 2 ready guardrails, 4 blocked
...
B4/B8 update: T-B4-002f/T-B8-003j turns that blocker into a real-backend transcript readiness guardrail. The gate consumes the fitted-spoofer holdout result plus the B10-T2 GenericBackendV2-style calibrated Aer bridge. It checks 10 readiness gates, passes 5, and fails 5: real backend properties R5, hardware execution R6, leakage-separated real-transcript fitting R7, leakage-blind no-leak acceptance below 0.10 R8, and full-private-material leakage bounded below 0.25 R9. The current stack still has 0 real backend transcript rows, leakage-blind no-leak fitted acceptance 0.35, and full-private-material leakage fitted acceptance 1.0, so it is not real-backend transcript readiness, protocol soundness, quantum advantage, or BQP separation.

B4/B8 update: T-B4-002g/T-B8-003k turns that failed readiness guardrail into a real-backend transcript evidence contract. The contract checks 10 requirements, passes 5, fails K5-K9, preserves source blockers R5-R9, and emits 5 PR packets: real backend properties, hardware execution, leakage-separated real fitting, leakage-blind no-leak margin, and full-leakage containment. This makes the remaining work explicit for human and AI-agent PRs, but it is still not real-backend readiness, protocol soundness, hardware evidence, quantum advantage, or BQP separation.

B4/B8 update: T-B4-002h/T-B8-003l adds a real-backend packet scout. The scout consumes the contract, readiness guardrail, and fitted-spoofer holdout evidence; checks 9 requirements; passes 4; and fails S5-S9. It maps all 5 backend PR packets and preserves 5760 backend-calibrated Aer circuits, 640 fitted evaluation rows, and 160 holdout rows, but real backend transcript rows are still 0, no real backend properties are used, no hardware execution is performed, and no leakage-separated real training exists. This keeps the B4/B8 route demoted while making the next evidence intake path auditable.
